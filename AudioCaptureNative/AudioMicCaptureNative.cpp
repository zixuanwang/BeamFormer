...
#include "AudioMicCaptureNative.h"
...
AudioMicCaptureNative::AudioMicCaptureNative(const wchar_t* captureDeviceID, 
    ...
    mIsInitialized(false)
{
    ...
    DMO_MEDIA_TYPE mt = {0};
    WAVEFORMATEXTENSIBLE* micWaveFormatEx = NULL;
    ...
    {
        hr = pEnumerator->GetDevice(captureDeviceID, &pDevice);
    }
    if (FAILED(hr))
    {
    ...
    // try target wave format with exclusive mode
    hr = mCaptureClient->Initialize(AUDCLNT_SHAREMODE_EXCLUSIVE,
                                    0,
    ...
    if (FAILED(hr))
    {
        // initialize with default wave format
        hr = mCaptureClient->GetMixFormat((WAVEFORMATEX**)&micWaveFormatEx);
        if (FAILED(hr))
        {
    ...
    if (micWaveFormatEx)
    {
        ...
        memcpy(mt.pbFormat, micWaveFormatEx, sizeof(WAVEFORMATEXTENSIBLE));
        ...
        memcpy(&mWaveFormatMic, micWaveFormatEx, sizeof(WAVEFORMATEX));
    }
    else
    {
        memcpy(&mWaveFormatMic, outWavFormat, sizeof(WAVEFORMATEX));
    }
...
AudioMicCaptureNative::~AudioMicCaptureNative()
{
...
void AudioMicCaptureNative::Start()
{
    if (!mIsInitialized)
    {
...
void AudioMicCaptureNative::Stop()
{
    if (!mIsInitialized)
    {
        throw std::logic_error("This instance is not properly initialized");
...
void AudioMicCaptureNative::GetData(byte* dataBuffer, int bufferSize, int* filledSize)
{
    HRESULT hr;
    ...
    *filledSize = 0;
    ...
    while (packetLength > 0)
    {
        ...
        {
            numBytesAvailable = numFramesAvailable * mWaveFormatMic.nBlockAlign;
...
bool AudioMicCaptureNative::IsCapturing() const
{
    if (!mIsInitialized)
    {
        throw std::logic_error("This instance is not properly initialized");
