#include "AudioMicArrayCaptureInterop.h"
...
AudioMicArrayCaptureInterop::~AudioMicArrayCaptureInterop()
{
	this->!AudioMicArrayCaptureInterop();
}
...
bool AudioMicArrayCaptureInterop::IsCapturing::get()
{
	return mMicCapture->IsCapturing();
}

void AudioMicArrayCaptureInterop::ThreadTask()
{
	...
	while (IsCapturing)
	{
		...
		if (bufFilled > 0)
		{
			mCallback(dataBuf, bufSize < bufFilled ? bufSize : bufFilled);
